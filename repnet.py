...
class ResnetPeriodEstimator(tf.keras.models.Model):
    ...
    def __init__(
        ...
        self.temporal_bn_layers = [
            layers.BatchNormalization() for _ in self.temporal_conv_layers
        ]
    ...
    def call(self, x):
        ...
        for bn_layer, conv_layer in zip(
            self.temporal_bn_layers, self.temporal_conv_layers
        ):
            x = conv_layer(x)
            x = bn_layer(x)
            x = tf.nn.relu(x)
        ...
        within_period_x = flatten_sequential_feats(
            within_period_x, batch_size, self.num_frames
        )
...
class TransformerLayer(tf.keras.layers.Layer):
  ...
  """
    ...
    def __init__(self, d_model, num_heads, dff, dropout_rate=0.1, reorder_ln=False):
...
def create_count_video(
  ...
  """
    ...
    if plot_count and not plot_within_period:
        ...
        if plot_score:
            plt.suptitle(
                "Pred Count: %d, " "Prob: %0.1f" % (int(np.around(final_count)), score),
                fontsize=24,
            )
        ...
        def update_within_period_plot(i):
            ...
            if plot_count:
                axs[0].set_title(
                    "Count %d, Rate: %0.4f Hz" % (count(i), rate(i)), fontsize=20
                )
            for idx in range(i):
                xs.append(idx)
...
def get_score(period_score, within_period_score):
    ...
    pred_period_conf = tf.reduce_max(tf.nn.softmax(period_score, axis=-1), axis=-1)
...
def get_counts(
    ...
    for stride in strides:
        ...
        within_period_score_stride = np.concatenate(within_period_score_stride, axis=0)
    ...
    raw_scores = np.repeat(raw_scores_list[argmax_strides], chosen_stride, axis=0)[
        :seq_len
    ]
    within_period = np.repeat(
        within_period_scores_list[argmax_strides], chosen_stride, axis=0
    )[:seq_len]
    ...
    if constant_speed:
        ...
        per_frame_counts = np.asarray(seq_len * [1.0 / pred_period]) * np.asarray(
            within_period_binary
        )
    else:
        ...
        per_frame_counts = tf.where(
            ...
            0.0,
            tf.math.divide(1.0, tf.cast(chosen_stride * per_frame_periods, tf.float32)),
        )
        if median_filter:
    ...
    if pred_score < threshold:
        print(
            "No repetitions detected in video as score "
            "%0.2f is less than threshold %0.2f." % (pred_score, threshold)
        )
        per_frame_counts = np.asarray(len(per_frame_counts) * [0.0])

    return (pred_period, pred_score, within_period, per_frame_counts, chosen_stride)
